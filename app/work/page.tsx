...
export default function WorkPage() {
  ...
  const FOCUS = translations[lang].workFocus
  ...
  return (
    <div ref={rootRef}>
              ...
              {t.heroTitle.map((line, i) => (
                <span key={i} style={{ display: 'block', overflow: 'hidden' }}>
                  <span className="sh-word" style={{ display: 'block' }}>
                    {i === 2 ? <><em style={{ fontStyle: 'italic', color: 'var(--cyan)' }}>{line.split(' ')[0]}</em>{' '}{line.split(' ').slice(1).join(' ')}</> : line}
                  </span>
                </span>
              ))}
            </h1>
      ...
      <section className="section">
        <div className="container">
          <div className="work-page-grid">
            <a href="https://athlos.fi" className="work-page-card" style={{ opacity: 0, display: 'block' }}>
              ...
              <div className="work-page-body">
                <span style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--cyan)', display: 'block', marginBottom: '0.375rem' }}>{t.customWebsite}</span>
                <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'white', marginBottom: '0.5rem' }}>Athlos Oy</h2>
                <p style={{ fontSize: '0.875rem', color: 'rgba(255,255,255,0.6)', marginBottom: '1rem' }}>{t.athlosDesc}</p>
                <span className="link-arrow link-arrow--light">{t.visitAthlos} <Arrow /></span>
              </div>
            </a>

            <a href="https://valuatum-com-rebuild.vercel.app/" target="_blank" rel="noreferrer" className="work-page-card" style={{ opacity: 0, display: 'block' }}>
              <Image src="/work-valuatum.png" alt="Valuatum website" fill sizes="50vw" style={{ objectFit: 'cover', objectPosition: 'top center' }} />
              <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(26,23,20,0.92), rgba(26,23,20,0.1) 50%, transparent)' }} />
              <div className="work-page-body">
                <span style={{ fontSize: '0.65rem', fontWeight: 700, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--cyan)', display: 'block', marginBottom: '0.375rem' }}>{t.websiteRebuild}</span>
                <h2 style={{ fontSize: '1.5rem', fontWeight: 700, color: 'white', marginBottom: '0.5rem' }}>Valuatum</h2>
                <p style={{ fontSize: '0.875rem', color: 'rgba(255,255,255,0.6)', marginBottom: '1rem' }}>{t.valuatumDesc}</p>
                <span className="link-arrow link-arrow--light">{t.viewProject} <Arrow /></span>
              </div>
            </a>
            ...
            <article className="work-page-card" style={{ opacity: 0 }}>
              ...
              <div style={{ position: 'absolute', inset: 0, background: 'rgba(26,23,20,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '0.5rem' }}>
                ...
                <p style={{ fontSize: '0.7rem', fontWeight: 700, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.3)' }}>{t.comingSoon}</p>
                <p style={{ color: 'rgba(255,255,255,0.2)', fontSize: '0.8125rem' }}>{t.moreProjectsSoon}</p>
              </div>
            </article>
          </div>
        </div>
      ...
      <section className="section section--surface">
        <div className="container">
          <p className="eyebrow reveal" style={{ marginBottom: '1rem' }}>{t.focusEyebrow}</p>
